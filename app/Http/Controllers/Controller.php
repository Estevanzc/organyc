...
abstract class Controller
{
    public function taxon_builder($specie_id)
    {
        ...
        return [
            "specie" => $specie,
            "genu" => $specie->genu,
            "family" => $specie->genu->family,
            "order" => $specie->genu->family->order,
            "class" => $specie->genu->family->order->class,
            "phylum" => $specie->genu->family->order->class->phylum,
            "kingdom" => $specie->genu->family->order->class->phylum->kingdom,
        ];
    }
    ...
    public function common_names($gbif_id)
    {
        ...
        return [
            $common_name[0],
            array_values(array_map(function ($name) {
                return explode(" ", strtolower($name[0]));
            }, $names_score))
        ];
    }
